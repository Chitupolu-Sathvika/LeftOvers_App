...
interface DonationFormProps {
   navigation: NativeStackNavigationProp<NavigationParamType, 'NewDonationForm'>;
}
...
const DonationForm: React.FC<DonationFormProps> = ({ navigation }) => {
   const [userData, setUserData] = useState<DonationDataType>({
      typeOfFood: '',
      cookedTime: '',
      quantity: 0,
      geoLocation: ''
   ...
   return (
      <ScrollView className={'bg-white flex flex-col gap-y-6 px-5'}>
         ...
         <View className={'flex flex-row justify-between'}>
            <View className={'w-[50%]'}>
               <Text className={'text-base'}>When the food is cooked</Text>
               <View className={'flex flex-row items-center gap-x-2 mt-3'}>
                  <TextInput
                     className={'bg-grey-4x-light w-[72px] px-5 py-4 rounded-[8px] text-lg'}
                     placeholder={'HH'}
                     onChangeText={(value) => (hours = parseInt(value))}
                     keyboardType={'numeric'}
                  />
                  <Text className={'text-3xl'}>:</Text>
                  <TextInput
                     className={'bg-grey-4x-light w-[76px] px-5 py-4 rounded-[8px] text-lg'}
                     placeholder={'MM'}
                     onChangeText={(value) => (minutes = parseInt(value))}
                     keyboardType={'numeric'}
                  />
               </View>
            </View>
